...
def generated_solver_exists(json_file, code_dir):
    return os.path.isfile(json_file) and os.path.isdir(code_dir)
...
def create_two_solvers(
    ...
    mem_all_before = get_process_memory_mb()
    print(f"[create_solver] before create_two_solvers | mem = {mem_all_before:.2f} MB")

    forward_code_dir = "c_generated_code_trailer_two_solver_sens_euler_w_fullseq"
    sens_code_dir = "c_generated_code_sensitivity_solver_euler_w_fullseq"

    forward_reuse = generated_solver_exists(forward_json, forward_code_dir)
    sens_reuse = generated_solver_exists(sens_json, sens_code_dir)

    if forward_reuse and sens_reuse:
        build_use = False
        generate_use = False
        print("[create_solver] generated solver files found, use build=False, generate=False")
    else:
        build_use = build
        generate_use = generate
        print("[create_solver] generated solver files not complete, build/generate solver")

    mem_fwd_ocp_before = get_process_memory_mb()
    print(f"[create_solver] before create forward OCP | mem = {mem_fwd_ocp_before:.2f} MB")

    ocp_fwd = create_ocp_forward(N)

    mem_fwd_ocp_after = get_process_memory_mb()
    print(
        f"[create_solver] after  create forward OCP | "
        f"mem = {mem_fwd_ocp_after:.2f} MB | "
        f"delta = {mem_fwd_ocp_after - mem_fwd_ocp_before:+.2f} MB"
    )

    mem_fwd_solver_before = get_process_memory_mb()
    print(f"[create_solver] before create forward solver | mem = {mem_fwd_solver_before:.2f} MB")

    ocp_solver = AcadosOcpSolver(
        ocp_fwd,
        build=build_use,
        generate=generate_use,
        json_file=forward_json,
        verbose=verbose,
    )

    mem_fwd_solver_after = get_process_memory_mb()
    print(
        f"[create_solver] after  create forward solver | "
        f"mem = {mem_fwd_solver_after:.2f} MB | "
        f"delta = {mem_fwd_solver_after - mem_fwd_solver_before:+.2f} MB"
    )

    mem_sens_ocp_before = get_process_memory_mb()
    print(f"[create_solver] before create sensitivity OCP | mem = {mem_sens_ocp_before:.2f} MB")
    ...
    mem_sens_ocp_after = get_process_memory_mb()
    print(
        f"[create_solver] after  create sensitivity OCP | "
        f"mem = {mem_sens_ocp_after:.2f} MB | "
        f"delta = {mem_sens_ocp_after - mem_sens_ocp_before:+.2f} MB"
    )

    mem_sens_solver_before = get_process_memory_mb()
    print(f"[create_solver] before create sensitivity solver | mem = {mem_sens_solver_before:.2f} MB")
    ...
    mem_sens_solver_after = get_process_memory_mb()
    print(
        f"[create_solver] after  create sensitivity solver | "
        f"mem = {mem_sens_solver_after:.2f} MB | "
        f"delta = {mem_sens_solver_after - mem_sens_solver_before:+.2f} MB"
    )

    mem_all_after = get_process_memory_mb()
    print(
        f"[create_solver] after create_two_solvers | "
        f"mem = {mem_all_after:.2f} MB | "
        f"total delta = {mem_all_after - mem_all_before:+.2f} MB"
    )
...
def load_target_record(load_path):
    if not os.path.exists(load_path):
        raise FileNotFoundError(
            f"{load_path} not found. Set GENERATE_TARGET_RECORD=True first."
        )
    ...
    rec = {
        "X_hist": data["X_hist"],
        "U_hist": data["U_hist"],
        "X_opt_hist": data["X_opt_hist"],
        "U_opt_hist": data["U_opt_hist"],
        "status_hist": data["status_hist"],
        "x0": data["x0"],
        "xref": data["xref"],
        "circle_c": data["circle_c"],
        "theta": data["theta"],
        "Tsim": int(data["Tsim"]),
        "N": int(data["N"]),
    }
...
def train_two_solver(
    ...
    # solver only created once here, then reused for all forward/backward calls
    ...
    for ep in range(epochs):
        ...
        for t in range(Tsim):
            ...
            print(
                f"[ep={ep + 1:03d}, t={t:04d}] before forward  | "
                f"mem = {mem_fwd_before:.2f} MB"
            )
            ...
            print(
                f"[ep={ep + 1:03d}, t={t:04d}] after  forward  | "
            ...
            print(
                f"[ep={ep + 1:03d}, t={t:04d}] before backward | "
                f"mem = {mem_bwd_before:.2f} MB"
            )
            ...
            print(
                f"[ep={ep + 1:03d}, t={t:04d}] after  backward | "
...
if __name__ == "__main__":
    ...
    # Hyperparameters
    # =========================================================
    GENERATE_TARGET_RECORD = True   # True: generate teacher record; False: load existing npz

    Tsim = 50
    N = 60

    target_record_file = "target_record_teacher_euler_w_fullseq.npz"
    ...
    # =========================================================
    # Target record: generate or load
    # =========================================================
    if GENERATE_TARGET_RECORD:
        print("\nGENERATE_TARGET_RECORD = True")
        print("Generating target record...")
        ...
        target_rec = generate_target_record(
            ...
            circle_c=circle_c,
            theta=theta_target,
            save_path=target_record_file,
        )

        print("\nTarget generated.")
    else:
        print("\nGENERATE_TARGET_RECORD = False")
        print(f"Loading target record from: {target_record_file}")

        target_rec = load_target_record(target_record_file)

        print("\nTarget loaded.")

    print("target status unique =", np.unique(target_rec["status_hist"]))
    print("target final state =", target_rec["X_hist"][-1])
    print("target theta =", target_rec["theta"])
    print("target Tsim =", target_rec["Tsim"])
    print("target N =", target_rec["N"])
